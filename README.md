...
* 不管方法有没有参数，通过方法表达式调用，第一个参数必须是接受者
...
Redis使用一个哈希表来保存所有的键值对，哈希桶中的元素保存的并不是值本身，而是指向具体值的指针。
哈希解决冲突办法：redis会对哈希表做rehash操作，增加现有的哈希桶数量。为了操作更高效，
默认使用两个全局哈希表。将哈希表2扩容，将哈希表1的数据重新映射并拷贝到哈希表2中，释放哈希表1。
每处理一个请求，从哈希表1第一个索引位置开始，拷贝索引位置上的entries拷贝到哈希表2中，依次进行。
...
* 简单动态字符串
    - 一个RedisObject包含8字节的元数据和8字节的指针，指针指向具体的实际数据。
    - Long类型整数，redisObject中的指针直接赋值为整数数据
    - 字符串数据且小于等于44字节，redisobject中的元数据、指针和SDS是一块连续的内存区域，是一块连续的内存区域
      减少内存碎片（embstr编码方式）。
    - 字符串数据且大于44字节，redisObject中的SDS分配独立内存空间，并用指针指向SDS
* 双向链表
* 压缩列表
    - 表头有三个字段：zlbytes(列表长度)、zltail（列表尾的偏移量）、zllen（列表中entry个数）
    - 表尾有一个zlend表示表结束
* 哈希表
* 跳表
...
redis单线程是指网络io和数据读写操作是由一个线程完成的。但是异步删除、持久化、数据同步是由额外的线程执行的。
